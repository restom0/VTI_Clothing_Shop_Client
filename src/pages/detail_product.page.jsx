...
const ProductDetailpage = () => {
  ...
  const mediaSlides = image.map((url, position) => ({
    alt: t("product.image_alt", { index: position + 1 }),
    key: `${url}-${position + 1}`,
    url,
  }));
    ...
    <section className={PRODUCT_DETAIL_PAGE_CLASSNAME}>
      <div className={PRODUCT_DETAIL_LAYOUT_CLASSNAME}>
        ...
        <div className={PRODUCT_INFO_COLUMN_CLASSNAME}>
          ...
          <div className={PRODUCT_OPTION_ROW_CLASSNAME}>
            {(product.object ?? []).map((color) => (
              <Tooltip
                content={color.product_id?.color_id?.color_name ?? ""}
                key={color.product_id.color_id.id}
              >
                <Button
                  size="lg"
                  variant="gradient"
            ...
            {product.object
              ...
              .map((size) => (
                <Tooltip
                  ...
                  key={size.product_id.size_id.id}
                >
                  <Button
                    size="md"
                    variant="gradient"
              ...
              {product.object
                ...
                .map((material) => (
                  <Radio
                    ...
                    key={material.product_id.material_id.id}
              ...
              <div>
                ...
                <div className="flex flex-col">
                  {stars.map((star, index) => (
                    <div className="w-full" key={5 - index}>
      ...
      <Container className="mt-5">
        <div>
          ...
          {reviews.slice(active * 4 - 4, active * 4).map((review) => (
            <Card key={review.id} className="mt-5">
